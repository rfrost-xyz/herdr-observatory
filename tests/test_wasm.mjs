...
const text=Array.from({length:36},(_,i)=>('ROW '+i+' WORKING agent / synthetic task ').padEnd(120,'.')).join('\n');
for(const name of catalogue)test(`full terminal ${name} completes with coloured cells`,()=>{const effect=createEffect(text,name,palette);let count=0,coloured=false,frame;try{while((frame=effect.next())){assert.equal(frame.width,120);assert.equal(frame.height,36);coloured ||= frame.fg.some(c=>c>>>24);assert.ok(++count<30000, 'effect must complete');}assert.ok(count>1);assert.ok(coloured);}finally{effect.free();effect.free();}assert.equal(effect.next(),null);});
