...
- Hidden/reduced-motion and asynchronous loading failure paths must release
  ...
  and current-state transcript. Keep host-role and theme-name labels out of the header; palette synchronisation remains active.
...
### Cumulative telemetry contract

`total_input`, `total_output`, `total_cache_read`, `total_cache_write` and `total_uncached_input` are optional numeric session counters, distinct from last-response fields. Codex uses native `total_token_usage`; Pi uses complete `getEntries()` snapshots capped at 4096 entries. Pi input excludes cache buckets at source, so include both cache buckets in total input while retaining source input as uncached. Cache values are tokens, never request hit/miss counts. Do not sum only a bounded tail and call it a session total.

`context_percent` uses the verified Codex v0.155.1 baseline-reserve percentage or Pi's reported extension percentage. `compactions` counts complete bounded saved history only; truncated/unavailable history is unknown. New fields pass through the same numeric allowlist, session binding, independent usage-source timestamps and Work revalidation. Preserve missing values and safe-integer bounds.

### Herdr metadata wire limits

Herdr accepts at most 16 token keys in one metadata report, retains at most 32 per pane and limits each value to 80 characters. Observatory wire v2 uses nine named identity/event/provenance fields plus four numeric groups (`obs_n0` through `obs_n3`), atomically in one 13-key report. The immutable order is `TELEMETRY_V2_GROUPS` in `probe.py`; each group holds at most four safe integers, comma-separated, with an empty component for unavailable data (maximum 67 characters). Change the wire version if this order changes. V2 decoding requires every group and rejects malformed values without falling back to old flat tokens. V1 remains readable. Old flat numeric keys may retain their previous TTL; they are ignored once v2 is present. Do not split reports with the same sequence: Herdr ignores subsequent equal-sequence writes. Tests must enforce native report/value limits, not just local roundtrip parsing.
...
Weekly remaining must come from a seven-day window selected by duration, not field position. A passed weekly reset invalidates the old balance, never refills it. Reset passes use native availableCount, not list length or purchased credit balance; earliest expiry requires complete valid available-pass detail. Expired passes invalidate the old count pending refresh. Optional unsupported data stays unknown. The footer renderer `web/allowances.mjs` independently guards stale/future samples and elapsed resets, uses textContent and stays within the opaque split footer. Keep both account panels bounded at 720p and 1080p.

Installed host extras for this optional feature are the image-supplied `allowances_probe.py` and `allowances-refresh.lock` timestamp next to the existing hook. The manual `codex_usage.py --refresh-allowances` command uses the same bounded capture without an agent session. Enabling/disabling account collection requires rerunning the adapter installer; changing mappings/publication requires updating private configuration and recreating the container. Keep real mappings and captured account summaries out of Git and public review evidence.
