...
Keep hook-time rollout enrichment for CLI threads. `thread/tokenUsage/updated` is documented for the app-server connection handling an active thread; launching a second short-lived app-server does not subscribe to an existing CLI thread. The adapter already emits one bounded numerical sample. The browser will deduplicate by pane, a collector-issued numeric session generation and usage sequence, calculate differences only from increasing cumulative counters, and hold a short bounded rolling series. Model and compaction observations are markers, not miss explanations. Pane replacement, source loss and counter discontinuities clear the baseline. No last-response count is added per hook. The generation changes when a valid Herdr session binding changes, without disclosing that binding.

No new identity or series enters presentation metadata. Opaque child and turn references remain on the source side of the reporting boundary; the adapter exposes a small validation seam for a future independently measured child source, without treating SubagentStop itself as a usage sample.
...
Build and verify the new image on the branch using synthetic telemetry and account activity. Keep the installed hook payload and previous versioned image available. The graphics repair is a separate open PR, so live rollout of this independent branch waits until that repair and this change are integrated; deploying this branch alone would regress graphics. At rollout, install the updated adapter through its idempotent installer and deploy the versioned image to both hosts. Verify account and cache source labels, responsive views and Work disclosure. Roll back by selecting the prior image and prior adapter payload if live acceptance fails. Archive and publish after local gates and independent review; merge only with explicit authorisation.
