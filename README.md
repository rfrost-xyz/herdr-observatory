...
All 37 bundled effects run locally through WebAssembly, using the same distribution served by Omarchy's website. A shuffled rotation visits every effect once before reshuffling and prevents consecutive repeats across rotations. Effects cover the complete 120×36 terminal and finish naturally; some full-screen effects run substantially longer than others. No animation frames are downloaded from the server. Only the current frame's typed cell buffers are retained.

The browser uses the existing filtered `/api/state` terminal text. Fresh samples continue to update accessible current state without replacing an animation. Source loss cancels obsolete playback. Space pauses; reduced motion shows the live TUI; hidden tabs freeze playback. After completion the live view holds for 10 seconds by default, adjustable with Left/Right in one-second increments (0–300). Page Up/Down selects thread pages. Native effect gradients use the active theme; fixed upstream accents are mapped into the theme palette while preserving brightness. Library names are not shown in the interface.
