...
The account cards put the weekly percentage and its remaining share bar first. `Room` divides the fresh weekly percentage remaining by the days until reset. `Burn` divides the weekly percentage already used by elapsed time in the seven-day window, after at least one hour. Both bars use the same scale within each card and show percentage points per day; an `Over pace` or `Within pace` cue compares them. These are guides from the observed allowance, not token balances or forecasts. Missing, expired or inconsistent window values leave the affected readings unavailable. The large daily token count and sparkline are omitted from the cards because account token activity cannot establish a remaining token quota. The bounded activity data remains in the existing private feed.
