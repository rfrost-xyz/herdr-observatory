...
The display is an unbranded modern character-cell TUI: rounded Unicode rules, glyph-led state rows, subtle theme-derived highlights and compact key hints. The bundled JetBrainsMono Nerd Font provides the same typography on Linux and Windows without a system font installation or external font request. Live text and CLI text effects use the same face. Font loading failure falls back to ordinary symbols and readable state labels; glyphs never replace the state words. Font source, checksum and licences live in `web/fonts/`. Its 140×44 grid uses smaller glyphs and side padding. Eight threads fit per page, with the total count and keyboard/automatic paging retained. A single 24-row CLI shows compact single-line noteworthy events (time, project, state, thread number and update): state changes, thread arrivals/departures and connection changes. Routine captures, pane dumps and revision/readiness changes add no log rows; available telemetry and R/L/F flags stay in the live table. Quiet sources leave the event stream quiet.

New CLI records scroll in as single lines. Eligible incoming lifecycle and supported telemetry status updates receive a text effect at that line's existing left edge and scrolling position. Other records remain readable, with no decorative project-name artwork or moving horizontal bands.

Effects finish naturally, then enforce a 120-second minimum cooldown by default. Left/Right adjusts this in one-second increments. Events received during playback or cooldown display immediately without being queued for later animation. Quiet periods do not replay previous output. The thread field uses Herdr’s native pane identifier (for example `w1E:p1`), also shown in the thread table. Pause freezes playback; reduced motion reveals plain text. The next effect is different and colours follow the active theme. No raw shell output is captured.
...
Thread rows use Herdr's Working, Blocked, Done, Idle and Unknown states. Working rows have independently phased text spinners; newly observed state or telemetry changes briefly highlight that row. Idle/done/unknown rows settle, and paused, reduced-motion or stale views suppress motion. All 37 text effects are available for eligible incoming status lines inside the 24-row events CLI, with live thread and machine information and controls remaining visible throughout playback. Refresh an already-open dashboard after an image update to load the latest browser code.
