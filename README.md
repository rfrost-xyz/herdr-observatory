...
The account cards show the reported weekly percentage and its remaining share bar, the scheduled reset, available reset passes and sample age. Pass expiry is available in the pass tooltip and accessible account summary. Missing or expired observations remain unknown until the account source reports again. The cards omit projected depletion times, pace charts, daily token totals and token sparklines because changing agent load makes a past-average projection unreliable and account token activity cannot establish a remaining token quota. Bounded token activity remains in the existing private feed.
