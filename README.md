...
`Room/day` divides the fresh weekly percentage remaining by the days until its scheduled reset. `Burn/day` divides the weekly percentage already used by elapsed time in the seven-day window, after at least one hour. Both are percentage points per day. The first is an even-use guide; the second is the average so far. Neither is a token balance or a forecast. Missing, expired or inconsistent window values leave the rates unavailable. Account token activity remains in its separate daily sparkline and is not converted into allowance units.
