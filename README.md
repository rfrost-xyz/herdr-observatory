...
The display is an unbranded modern character-cell TUI: rounded Unicode rules, glyph-led state rows, subtle theme-derived highlights and compact key hints. The bundled JetBrainsMono Nerd Font provides the same typography on Linux and Windows without a system font installation or external font request. Live text and whole-terminal effects use the same face. Font loading failure falls back to ordinary symbols and readable state labels; glyphs never replace the state words. Font source, checksum and licences live in `web/fonts/`. Its 140×44 grid uses smaller glyphs and side padding. The lower ten-line CLI feed scrolls as fresh samples and observations arrive; every line has a UTC observation timestamp and type. Status changes stamp the known before/after state once. R/L/F indicators (ready, launching and focused, with a visible legend) expose Herdr's existing typed flags; revision and state-sequence changes are explicitly observed metadata, not invented tool calls. A short event register retains recent milestones above the rolling feed.

Delta Corps Priest 1 artwork appears only for a fresh DONE or INPUT transition: six seconds, at most once per thirty seconds. It is live-only; full-scene effects capture the text event register instead so an animation cannot prolong a temporary banner. It occupies its own area below the thread table, never overlays current threads, and is suppressed for reduced motion or paused FX. Pending banners expire after thirty seconds or when their source/state is no longer current. Attribution is in `web/vendor/STAMPS-NOTICE`. Whole-terminal coloured text effects animate a permitted snapshot, including the header, sources, threads and feed. Each effect plays through its final frame, then the latest live TUI remains visible for the configured hold. The next effect is different. Colours follow the active desktop theme. Pause freezes playback; reduced motion keeps the live TUI readable. No raw shell output is captured.
...
All 37 bundled effects run locally through WebAssembly, using the same distribution served by Omarchy's website. A shuffled rotation visits every effect once before reshuffling and prevents consecutive repeats across rotations. Effects cover the complete 140×44 terminal and finish naturally; some full-screen effects run substantially longer than others. No animation frames are downloaded from the server. Only the current frame's typed cell buffers are retained.

The browser composes live and animated scenes from the same filtered `/api/state` fields and its bounded timestamped observation records; the server also provides a bounded plain terminal layout. Fresh samples continue to update accessible current state without replacing an animation. Source loss cancels obsolete playback. Space pauses; reduced motion shows the live TUI; hidden tabs freeze playback. After completion the live view holds for 10 seconds by default, adjustable with Left/Right in one-second increments (0–300). Page Up/Down selects thread pages. Native effect gradients use the active theme; fixed upstream accents are mapped into the theme palette while preserving brightness. Library names are not shown in the interface.
...
Run `python -m unittest discover -s tests -v`, `node --check web/app.js`, `node --test tests/test_ui.cjs tests/test_wasm.mjs`, and `openspec validate --all --strict`. The WASM tests verify artifact hashes and run every effect to completion using a full-screen synthetic terminal.

### Further Herdr API coverage

The installed local Herdr 0.9.1 schema (protocol 22) exposes `interactive_ready`, `launch_pending`, `focused`, `revision` and `state_change_seq`; this display now uses those existing sanitised fields. Status transitions are discovered by polling, so rapid intermediate transitions can be missed. STATESEQ rows report this limitation explicitly. Timestamps denote observation time, not an asserted original event time. No token throughput or tool execution is inferred.
