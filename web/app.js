...
function buildView(){
  cardNodes=Array.from({length:8},(_,index)=>{
    ...
    const detail=node('div','thread-meta'),freshness=node('p','thread-freshness'),compactionAge=node('span','freshness-part'),hookAge=node('span','freshness-part'),usageAge=node('span','freshness-part'),accessibleMetrics=node('span','sr-only');accessibleMetrics.id=`thread-metrics-${index}`;detail.append(identity,model);freshness.append(compactionAge,hookAge,usageAge);card.append(head,checkout,detail,activity,metrics,pending,recent,freshness,coverage,accessibleMetrics);document.getElementById('threads').append(card);return {card,project,glyph,state,checkout,identity,detail,freshness,compactionAge,hookAge,usageAge,activity,activityGlyph,activityText,tool,model,metrics,pending,pendingText,recent,coverage,accessibleMetrics,fields};
  });
