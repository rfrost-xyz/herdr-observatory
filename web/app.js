...
function buildView(){
  cardNodes=Array.from({length:8},(_,index)=>{
    ...
    const detail=node('div','thread-meta'),footer=node('div','card-footer'),freshness=node('p','thread-freshness'),compactionAge=node('span','freshness-part'),hookAge=node('span','freshness-part'),usageAge=node('span','freshness-part'),accessibleMetrics=node('span','sr-only');accessibleMetrics.id=`thread-metrics-${index}`;projectPanel.append(project,checkout);statePanel.append(status,activity);head.append(projectPanel,statePanel);detail.append(identity,model,coverage);freshness.append(compactionAge,hookAge,usageAge);footer.append(detail,freshness);card.append(head,metrics,pending,recent,footer,accessibleMetrics);document.getElementById('threads').append(card);return {card,project,glyph,state:status,checkout,identity,detail,footer,freshness,compactionAge,hookAge,usageAge,activity,activityText,tool,model,metrics,pending,pendingText,recent,coverage,accessibleMetrics,fields};
  });
...
function draw(now=performance.now()) {
  ...
  cardNodes.forEach((card,i)=>{const model=view.cards[i];card.card.hidden=!model;if(!model){for(const field of ['project','checkout','identity','activityText','tool','model','recent','coverage','accessibleMetrics','compactionAge','hookAge','usageAge']){setText(card[field],'');card[field].title='';}card.freshness.title='';card.freshness.setAttribute('aria-label','');for(const f of card.fields){for(const part of ['label','value','exact'])setText(f[part],'');f.tile.title='';f.tile.setAttribute('aria-label','');}card.state.setAttribute('aria-label','');card.state.title='';setText(card.glyph,'');card.card.setAttribute('aria-label','');return;}
