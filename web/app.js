...
function viewModel(now=performance.now(),wall=Date.now()) {
  ...
  return {
    ...
    const detail=cardTelemetry(a,t),tiles=detail.tiles.map(tile=>({...tile,lastKnown:tile.source==='usage'?usageLastKnown:hookLastKnown}));
    const childCounts=a.harness==='codex' && Number.isSafeInteger(t?.subagent_starts) && Number.isSafeInteger(t?.subagent_stops) && Number.isSafeInteger(t?.subagent_seq) && t.subagent_seq>0 && t.subagent_seq<=t.seq && t.subagent_starts>=0 && t.subagent_starts<=999 && t.subagent_stops>=0 && t.subagent_stops<=999;
    const childAge=childCounts?ageLabel(wall-t.subagent_seq/1000):null;
    const childLastKnown=childCounts && wall-t.subagent_seq/1000>120000;
    const subagentSummary=childCounts?{label:`Observed subagents${childLastKnown?' (last known)':''} · ${t.subagent_starts} starts · ${t.subagent_stops} stops`,detail:`Observed this parent turn: ${t.subagent_starts} start hooks and ${t.subagent_stops} stop hooks; ${childLastKnown?'last known, ':''}last subagent hook ${childAge}. Counts may be incomplete. Stops do not establish successful completion or a live child count.`,lastKnown:childLastKnown}:null;
    return {
      ...
      tiles,instruments:visibleInstruments(tiles),recentCache:a.harness==='codex'?recentCache(a.id):null,subagentSummary
    };}),
...
function buildView(){
  cardNodes=Array.from({length:8},(_,index)=>{
    ...
    const subagents=node('div','subagent-summary');
    const detail=node('div','thread-meta'),footer=node('div','card-footer'),accessibleMetrics=node('span','sr-only');accessibleMetrics.id=`thread-metrics-${index}`;projectPanel.append(project,checkout);statePanel.append(status);head.append(projectPanel,statePanel,activity,subagents);detail.append(identity);footer.append(detail);card.append(head,metrics,pending,recent,footer,accessibleMetrics);document.getElementById('threads').append(card);return {card,project,glyph,state:status,checkout,identity,detail,footer,activity,activityText,tool,subagents,metrics,pending,pendingText,recent,accessibleMetrics,fields};
  });
...
function draw(now=performance.now()) {
  ...
  cardNodes.forEach((card,i)=>{const model=view.cards[i];card.card.hidden=!model;if(!model){for(const field of ['project','checkout','identity','activityText','tool','subagents','recent','accessibleMetrics']){setText(card[field],'');card[field].title='';}for(const f of card.fields){for(const part of ['label','value','exact'])setText(f[part],'');f.tile.title='';f.tile.setAttribute('aria-label','');}card.state.setAttribute('aria-label','');card.state.title='';setText(card.glyph,'');card.card.setAttribute('aria-label','');return;}
    ...
    card.activity.hidden=!model.activity && !model.tool;card.activity.dataset.historical=String(model.historicalActivity);card.activity.title=model.historicalActivity?`${model.activity}${model.tool?` · ${model.tool}`:''}`:'Latest observed hook activity; the state icon is Herdr’s current state';setText(card.activityText,model.activity || (model.tool?'Tool observed':''));card.activityText.hidden=!model.activity && !model.tool;setText(card.tool,model.tool||'');card.tool.hidden=!model.tool;card.tool.title=model.tool;setText(card.subagents,model.subagentSummary?.label || '');card.subagents.hidden=!model.subagentSummary;card.subagents.title=model.subagentSummary?.detail || '';card.subagents.dataset.lastKnown=String(Boolean(model.subagentSummary?.lastKnown));
