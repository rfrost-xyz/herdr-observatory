...
function cardTelemetry(a,t){
  ...
  if(has('input') || has('output_tokens'))tiles.push({kind:'response',source:usageSource,label:'Last response',value:`↓ ${compactTokens(t.input)}  ↑ ${compactTokens(t.output_tokens)}`,detail:`${groupedNumber(t.input)} input / ${groupedNumber(t.output_tokens)} output tokens (last response)`,inputTokens:has('input')?t.input:null});
...
function visibleInstruments(tiles){
  ...
  const session=find('session') || find('response');
  const tokens=session?{...session,value:session.value.replace('  ↑','\n↑'),flowIn:session.value.split('  ↑')[0],flowOut:`↑${session.value.split('  ↑')[1]??' —'}`}: {kind:'session',source:'hook',label:'Session tokens',value:'—',ratio:null,detail:'Session token totals unavailable',lastKnown:false};
...
function buildView(){
  cardNodes=Array.from({length:8},(_,index)=>{
    const card=node('article','thread-card'),head=node('div','card-top'),project=node('h3','project'),statusGroup=node('div','status-group'),status=node('span','state'),glyph=node('span','state-glyph'),state=node('span','state-word');status.append(glyph,state);
    ...
    const fields=Array.from({length:3},()=>{const tile=node('div','usage-tile'),label=node('span','usage-label'),value=node('strong','usage-value'),exact=node('small','usage-exact'),bar=node('span','usage-bar'),fill=node('i',''),remainder=node('i','usage-remainder'),write=node('i','usage-write');tile.setAttribute('role','group');bar.append(fill,remainder,write);bar.setAttribute('aria-hidden','true');tile.append(label,value,exact,bar);metrics.append(tile);return {tile,label,value,exact,bar,fill,remainder,write};});
    const detail=node('div','thread-meta'),freshness=node('p','thread-freshness'),compactionAge=node('span','freshness-part'),hookAge=node('span','freshness-part'),usageAge=node('span','freshness-part'),accessibleMetrics=node('span','sr-only');accessibleMetrics.id=`thread-metrics-${index}`;head.append(project,identity);detail.append(model);statusGroup.append(status,activity);freshness.append(compactionAge,hookAge,usageAge);card.append(head,checkout,detail,statusGroup,metrics,pending,recent,freshness,coverage,accessibleMetrics);document.getElementById('threads').append(card);return {card,project,glyph,state,checkout,identity,detail,freshness,compactionAge,hookAge,usageAge,activity,activityGlyph,activityText,tool,model,metrics,pending,pendingText,recent,coverage,accessibleMetrics,fields};
  });
...
function draw(now=performance.now()) {
  ...
  cardNodes.forEach((card,i)=>{const model=view.cards[i];card.card.hidden=!model;if(!model){for(const field of ['project','state','checkout','identity','activityText','tool','model','recent','coverage','accessibleMetrics','compactionAge','hookAge','usageAge']){setText(card[field],'');card[field].title='';}card.freshness.title='';card.freshness.setAttribute('aria-label','');for(const f of card.fields){for(const part of ['label','value','exact'])setText(f[part],'');f.tile.title='';f.tile.setAttribute('aria-label','');}card.card.setAttribute('aria-label','');return;}
    ...
    card.metrics.hidden=!model.instruments.length;card.pending.hidden=Boolean(model.instruments.length);setText(card.pendingText,model.note==='No hook sample'?'No hook sample':'Usage pending');card.pending.title=model.note;card.fields.forEach((f,j)=>{const tile=model.instruments[j];f.tile.hidden=!tile;if(!tile)return;f.tile.dataset.kind=tile.kind;f.tile.dataset.lastKnown=String(tile.lastKnown);f.tile.dataset.known=String(tile.ratio!=null);f.tile.dataset.flow=String(Boolean(tile.flowIn));f.tile.style.setProperty('--ratio',`${Math.min(1,Math.max(0,tile.ratio??0))*100}%`);setText(f.label,tile.label);setText(f.value,tile.value);f.value.setAttribute('data-compact',tile.kind==='context'?tile.value.split(' ')[0]:'');f.value.setAttribute('data-input',tile.flowIn||'');f.value.setAttribute('data-output',tile.flowOut||'');setText(f.exact,tile.exact || '');f.exact.hidden=!tile.exact;f.tile.title=tile.detail;f.tile.setAttribute('aria-label',`${tile.lastKnown?'Last known. ':''}${tile.label}: ${tile.detail}`);f.bar.hidden=true;});
