...
function cardTelemetry(a,t){
  ...
  if(has('total_input') || has('total_output'))tiles.push({kind:'session',label:'Session tokens',value:`↓ ${compactTokens(t.total_input)}  ↑ ${compactTokens(t.total_output)}`,detail:`${groupedNumber(t.total_input)} input / ${groupedNumber(t.total_output)} output tokens (session)`});
  if(has('total_cache_read') || has('total_uncached_input')){
...
function buildView(){
  cardNodes=Array.from({length:8},(_,index)=>{
    ...
    const fields=Array.from({length:4},()=>{const tile=node('div','usage-tile'),label=node('span','usage-label'),value=node('strong','usage-value'),exact=node('small','usage-exact'),bar=node('span','usage-bar'),fill=node('i',''),remainder=node('i','usage-remainder');tile.setAttribute('role','group');bar.append(fill,remainder);bar.setAttribute('aria-hidden','true');tile.append(label,value,exact,bar);metrics.append(tile);return {tile,label,value,exact,bar,fill,remainder};});
    const detail=node('div','thread-meta'),freshness=node('p','thread-freshness'),accessibleMetrics=node('span','sr-only');accessibleMetrics.id=`thread-metrics-${index}`;detail.append(identity,model);card.append(head,checkout,detail,activity,metrics,freshness,coverage,accessibleMetrics);document.getElementById('threads').append(card);return {card,project,glyph,state,checkout,identity,detail,freshness,activity,activityGlyph,activityText,tool,model,metrics,coverage,accessibleMetrics,fields};
  });
