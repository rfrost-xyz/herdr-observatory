...
function ThreadCard({model, index, now, cardClicks, disconnected, reduced, icon, cacheTrend, tokenPercent, groupedNumber, onPulse}) {
  ...
  return <article className="thread-card" data-state={model.state.toLowerCase()} data-thread={model.id} role="button" tabIndex="0"
    ...
    <div className="card-footer"><div className="thread-meta"><p className="identity" title={`Host: ${model.host} · Herdr pane: ${model.pane} · Harness: ${model.harness} · Model: ${model.model || 'unavailable'}`}>{model.host} · {model.pane} · {model.harness} · {model.model || '—'}</p></div>
      {model.note && model.tiles.length>0 && <p className="coverage" title={model.note}>{model.note==='Partial usage coverage'?'Partial coverage':model.note}</p>}
