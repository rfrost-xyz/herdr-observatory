...
function ThreadCard({model, index, now, cardClicks, disconnected, reduced, icon, cacheTrend, tokenPercent, groupedNumber, onPulse}) {
  ...
  return <article className="thread-card" data-state={model.state.toLowerCase()} data-thread={model.id} role="button" tabIndex="0"
    ...
    <div className="card-top"><div className="project-panel"><h3 className="project" title={model.project}>{model.project}</h3>{checkout && <p className="checkout" title={`Worktree / checkout: ${model.checkout}`}>{icon('branch')} {checkout}</p>}</div>
      <div className="state-panel"><span className="state" role="img" aria-label={model.state} title={model.state}><span className="state-glyph" aria-hidden="true">{model.motion.moving?<Blocks size={30} color="var(--state)" playState="running"/>:model.motion.glyph}</span></span>
        {(model.activity||model.tool) && <div className="card-activity" data-historical={String(model.historicalActivity)} title={model.historicalActivity?`${model.activity}${model.tool?` · ${model.tool}`:''}`:'Latest observed hook activity; the state icon is Herdr’s current state'}><strong className="activity-text">{model.activity||'Tool observed'}</strong>{model.tool && <span className="tool-name" title={model.tool}>{model.tool}</span>}</div>}</div></div>
    ...
    <div className="card-footer"><div className="thread-meta"><p className="identity" title={`Harness: ${model.harness} · Host: ${model.host} · Pane: ${model.pane}`}>{model.harness} · {model.host} · {model.pane}</p>
      {model.model && <p className="model-name" title={model.model}>{model.model}</p>}{model.note && model.tiles.length>0 && <p className="coverage" title={model.note}>{model.note==='Partial usage coverage'?'Partial coverage':model.note}</p>}</div>
      <p className="thread-freshness" aria-label={[model.compactions?`Compactions ${model.compactions.value}`:null,model.freshness,model.usageFreshness].filter(Boolean).join('. ')} title={[model.compactions?.detail,model.usageAge?`Usage source: ${model.usageSource}`:null].filter(Boolean).join(' · ')}>{model.compactions && <span className="freshness-part" data-short={`C ${model.compactions.value}`}>Compactions {model.compactions.value}</span>}<span className="freshness-part" data-short={`H ${model.hookAge?.replace(' ago','')||'?'}`}>{model.freshness}</span><span className="freshness-part" data-short={`U ${model.usageAge?.replace(' ago','')||'?'}`}>{model.usageFreshness}</span></p></div>
